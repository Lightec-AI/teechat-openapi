...
| `OPENAPI_ENGINE_IDENTITY_PINS_JSON` | Legacy anchor, required with OPE dispatch **unless** `OPENAPI_OPE_REQUIRE_EPOCH_EVIDENCE=1` or `OPENAPI_STAGE5_IDENTITY_DELETED=1`: JSON map of engine id to attestation-approved Ed25519 identity key; its canonical SHA-256 is included in `policy_hash` |
...
| `OPENAPI_STAGE5_IDENTITY_DELETED` | RB-52 Stage 5. **Default unset/off.** When `1`, empty or stale identity pins do not admit and `OPE-ENGINE-EPHEMERAL-v1` is not an admit path. Do not set on live until leftover ≤0.24.0 census is clean and an explicit GO |
