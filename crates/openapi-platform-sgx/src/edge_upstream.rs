//! Edge upstream selection — hard OPE cutover for Fortanix SGX (clear HTTP only as
//! non-prod break-glass). Same decision tree as `openapi-platform-cvm::edge_upstream`;
//! the `Clear` arm uses the SGX `TcpHttpUpstream` (plain `TcpStream`, `http://IP:port`)
//! instead of CVM's `ureq`-based `UreqUpstream`.
...
use crate::gateway_ope_api::require_gateway_ope_api_healthy;
use crate::ope_upstream::{clear_http_break_glass_enabled, OpeDispatchUpstream};
use crate::upstream::TcpHttpUpstream;

pub enum EdgeUpstream {
    Ope(OpeDispatchUpstream),
    Clear(TcpHttpUpstream),
}
...
impl EdgeUpstream {
    ...
    pub fn from_env(profile: EdgeProfile, clear_base_url: &str) -> anyhow::Result<Self> {
        ...
        if break_glass {
            ...
            return Ok(Self::Clear(TcpHttpUpstream::new(clear_base_url)?));
        }
        ...
        match OpeDispatchUpstream::from_env() {
            Ok(Some(ope)) => match require_gateway_ope_api_healthy(profile) {
                ...
                Err(e) => {
                    ...
                    Ok(Self::Clear(TcpHttpUpstream::new(clear_base_url)?))
                }
            },
