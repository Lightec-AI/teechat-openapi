...
mod pkcs8_serde {
    ...
    where
        S: Serializer,
    {
        let encoded = URL_SAFE_NO_PAD.encode(key_pkcs8.as_ref());
    ...
    pub(crate) fn deserialize<'de, D: Deserializer<'de>>(
        ...
        impl<'de> de::Visitor<'de> for Visitor {
            ...
            where
                E: de::Error,
            {
                URL_SAFE_NO_PAD.decode(v).map_err(de::Error::custom)
            }
        }
...
impl<'a> Serialize for RevocationRequest<'a> {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        let base64 = URL_SAFE_NO_PAD.encode(self.certificate);
...
fn base64(data: &impl Serialize) -> Result<String, serde_json::Error> {
    Ok(URL_SAFE_NO_PAD.encode(serde_json::to_vec(data)?))
}
